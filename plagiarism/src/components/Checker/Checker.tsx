...
function Checker() {
  ...
  const [inputUpload, setInput] = useState<boolean>(false);
  const [datasetUpload, setDataset] = useState<boolean>(false);
  const [showtext, setText] = useState<boolean>(false);
  const [getinputText, setInputtext] = useState<string>('');
  const [loading, setLoading] = useState<boolean>(false);
  const { setContextData, setDatasetTextData, setdatasetimgData, setinputimgData, setInputonFiles, setDatasetonFiles } = useDataContext();
  ...
  const ongetText = async () => {
    try {
      ...
      const inputText: string[] = inputTextResponse.data.inputText;
      const datasettext: string = inputTextResponse.data.datasetText;
  ...
  return (
    <div className="checker_container">
      <div className="dropzone">
        {showtext ? (
          <div style={{ color: 'white' }}>{getinputText}</div>
        ) : (
          <></>
        )}
        {inputUpload ? (
